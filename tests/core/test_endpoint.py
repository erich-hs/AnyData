...
def test_set_path_params_no_param_values(endpoint_with_path_params_and_no_param_values):
    ...
    assert list(endpoint_with_path_params_and_no_param_values.path_params.keys()) == [
        "year",
        "state",
    ]
    assert list(endpoint_with_path_params_and_no_param_values.path_params.values()) == [
        None,
        None,
    ]
...
def test_merge_headers(valid_endpoint):
    valid_endpoint.merge_headers(
        {"header1": "value1", "Connection": "close", "Accept": "application/json"}
    )
    ...
    assert all(
        [
            header in valid_endpoint.headers
...
def test_endpoint_default_headers(valid_endpoint):
    default_header_params = ["User-Agent", "Accept-Encoding", "Accept", "Connection"]
